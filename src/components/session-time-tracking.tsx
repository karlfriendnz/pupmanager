...
function EntryForm({
  ...
  return (
    // No card of its own. This form lives inside the time screen's single
    // bordered block, and a bordered, tinted panel inside a bordered block
    // inside a padded page was three frames around four fields (Karl: "the log
    // time has way too many containers").
...
export function SessionTimeTracking({
  ...
  // You got here by pressing "Log time", so the form is already open when
  // there is nothing to look at yet. Pressing a second button to reach the
  // thing the first button named is a step that says nothing.
  const [adding, setAdding] = useState(initialEntries.length === 0)
