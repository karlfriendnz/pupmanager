  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <label className="mt-4 block text-xs font-medium text-slate-700">1:1 session (optional)</label>
        <select
          value={cfg.packageId ?? ''}
        ...
        >
          <option value="">Standalone session (not part of a 1:1 Sessions offering)</option>
          {packages.map(p => (
        ...
        <p className="mt-1.5 text-xs text-slate-400">
          {pkg
            ? `The chosen time becomes session 1; the remaining ${Math.max(0, pkg.sessionCount - 1)} auto-schedule on the 1:1 session cadence.`
        ...
        <div className="mt-5 flex items-start justify-between gap-4 border-t border-slate-100 pt-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              {cfg.packageId
                ? 'Charges the 1:1 session price. The slot is booked once paid.'
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              {cfg.packageId
                ...
                : 'Single-session bookings by existing clients always book instantly. New people are reviewed when you accept their enquiry. Attach a 1:1 session to require approval.'}
            </p>
          </div>
          {cfg.packageId ? (
