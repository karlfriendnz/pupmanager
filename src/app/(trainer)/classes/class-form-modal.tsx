...
export function ClassFormModal({
  ...
  async function handleConvertToPackage() {
    ...
    if (!confirm('Convert this class back into a 1:1 session? Its scheduled sessions will be removed.')) return
    setConverting(true)
            ...
            <div className="rounded-xl border border-slate-200 px-3 py-2.5 flex items-start gap-3">
              <span className="flex-1 min-w-0">
                ...
                <span className="block text-[11px] text-slate-400 mt-0.5">
                  Turn it back into a 1:1 session. The scheduled sessions are removed — only possible while nobody is booked in.
                </span>
                {convertError && (
              ...
              >
                {converting ? 'Converting…' : 'Convert to 1:1 session'}
              </button>
            </div>
          )}
