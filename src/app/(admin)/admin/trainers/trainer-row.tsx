...
// A single desktop-table trainer row. Deliberately light: it shows the
// at-a-glance fields and taps through to the trainer's full view, where every
// detail + action lives. The only inline shortcut is "log in as" (impersonate),
// which stops row propagation so it doesn't also open the detail.
type Trainer = {
  id: string
  name: string | null
  email: string
    ...
    >
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          {trainer.flag && (
            <span aria-hidden className="text-sm leading-none" title={`Signed up in ${trainer.signupCountry}`}>{trainer.flag}</span>
          )}
          {trainer.isInternal && (
