...
export async function TrainersTable({
  ...
  const trainers = await prisma.user.findMany({
    where: {
      ...
      ...(statuses && statuses.length ? { trainerProfile: { subscriptionStatus: { in: statuses } } } : {}),
      // "Not on a paying plan" = no ACTIVE subscription. NOT on the relation
      // also catches trainers with no profile or a null status (never excluded
      // by a plain `not: 'ACTIVE'`, which drops NULLs).
      ...(onlyNonPaying ? { NOT: { trainerProfile: { subscriptionStatus: 'ACTIVE' } } } : {}),
    },
