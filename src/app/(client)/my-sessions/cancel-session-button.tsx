...
// Cancel affordance for one upcoming 1:1 session. Opens a confirm sheet that
// shows the cancellation fee BEFORE the client commits (feeCents is computed
// server-side from the trainer's config for this session's start time).
export function CancelSessionButton({
  ...
  async function confirm() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/my/sessions/${sessionId}/cancel`, { method: 'POST' })
      if (res.ok) {
        setOpen(false)
        router.refresh()
      } else {
        const b = await res.json().catch(() => ({}))
        setError(typeof b.error === 'string' ? b.error : 'Could not cancel this session.')
      }
    } catch {
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 px-4 py-6" onClick={() => !busy && setOpen(false)}>
          <div className="w-full max-w-sm rounded-3xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3">
              <h3 className="font-display text-lg font-bold text-slate-900">Cancel this session?</h3>
            ...
            {feeStr && (
              <div className="mt-4 flex items-start gap-2.5 rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-800">
                ...
                <span>Cancelling now incurs a <strong>{feeStr}</strong> cancellation fee.</span>
              </div>
            )}
            ...
            <div className="mt-5 flex gap-2.5">
              ...
              >
                {busy ? 'Cancelling…' : feeStr ? `Cancel & pay ${feeStr}` : 'Cancel session'}
              </button>
            </div>
          </div>
        </div>
