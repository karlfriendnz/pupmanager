...
describe('onboarding email reply-to', () => {
  it('replies go to the shared support inbox', () => {
    expect(renderOnboardingEmail(tmpl, ctx).replyTo).toBe('info@pupmanager.com')
  })

  it('reply-to is the support inbox regardless of which founder voice sent it', () => {
    expect(renderOnboardingEmail({ ...tmpl, senderKey: 'brooke' }, ctx).replyTo).toBe('info@pupmanager.com')
    expect(renderOnboardingEmail({ ...tmpl, senderKey: 'someone-else' }, ctx).replyTo).toBe('info@pupmanager.com')
  })
