...
// The client branch counts their 1:1 thread PLUS their groups with the same
// business, so it goes through countClientUnreadTotalForProfile; the trainer
// branch is unchanged.
const h = vi.hoisted(() => ({
  auth: vi.fn(),
  getActiveClient: vi.fn(),
  countUnread: vi.fn(),
  countClientTotal: vi.fn(),
}))
...
vi.mock('@/lib/client-message-threads', () => ({
  countClientUnreadTotalForProfile: h.countClientTotal,
}))
...
beforeEach(() => {
  ...
  h.countClientTotal.mockResolvedValue(5)
})
...
describe('GET /api/messages/unread-count', () => {
  it('401s an unauthenticated request', async () => {
    ...
    expect(h.countClientTotal).not.toHaveBeenCalled()
  })

  it('client: returns their active thread + group unread, scoped to the active clientId', async () => {
    ...
    expect(h.countClientTotal).toHaveBeenCalledWith({ clientId: 'cp-1', userId: 'cu-1' })
    // Never the trainer scope — a client query must not widen to a company.
    expect(h.countUnread).not.toHaveBeenCalled()
  })
