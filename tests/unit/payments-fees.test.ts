...
describe('platformFeeAmount — our margin on a client payment', () => {
  // We take a flat 1% (Karl, 2026-07-28). It used to be a per-currency spread
  // reverse-engineered to land the all-in on 3.5% everywhere, which charged GBP
  // trainers 2% against a promised 2.5%. See platform-fee.test.ts, which pins
  // each all-in figure against the rates the pricing page advertises.
...
describe('estimateProcessingSurcharge — the client covers BOTH fees', () => {
  ...
  it('nets the trainer EXACTLY the item price, in every currency', () => {
    ...
    const cases: Array<[number, string, number, number, number]> = [
      [10_000, 'nzd', 265, 30, 100],
      [10_000, 'aud', 170, 30, 100],
      [5_000, 'gbp', 150, 20, 100],
      [10_000, 'usd', 290, 30, 100],
      [25_000, 'cad', 290, 30, 100],
    ]
  ...
  it('covers our margin too — a surcharge that only paid Stripe would short the trainer', () => {
    // AUD is the case that was broken: it grossed up at Stripe's rate alone
    // while the true cost is Stripe 1.7% + our 1% = 2.7%, so the trainer lost
    // our margin out of their own pocket on every payment.
