...
describe('application_fee_percent', () => {
  it('converts each currency margin from basis points to percent', () => {
    expect(applicationFeePercentFor('nzd')).toBe(0.85)
    expect(applicationFeePercentFor('aud')).toBe(1.8)
    expect(applicationFeePercentFor('gbp')).toBe(2)
    expect(applicationFeePercentFor('usd')).toBe(0.6)
    expect(applicationFeePercentFor('cad')).toBe(0.6)
  })

  it('returns null for zar so the field is OMITTED, never sent as 0', () => {
    // Stripe rejects a 0 application fee, and the one-off path already omits it
    // for the same reason. This is the assertion that keeps the two consistent.
    expect(applicationFeePercentFor('zar')).toBeNull()
  })

  it('returns null for a currency whose margin we have never confirmed', () => {
    expect(applicationFeePercentFor('jpy')).toBeNull()
  })
})
